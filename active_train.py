...
def _save_q_error_outputs(args, size, records, fold_summaries):
	...
	return q_error_path, summary_path
...
def pretrain_finetune_experiment(args):
	...
	for size in sorted(finetune_folds.keys()):
		...
		for fold_idx, (train_queries, val_queries, test_queries) in enumerate(folds):
			...
			if test_records and fold_summary is not None:
				print("Fold {} test q_error mean: {:.4f}, median: {:.4f}, p25: {:.4f}, p75: {:.4f}".format(
					fold_idx + 1,
					fold_summary["mean"],
					fold_summary["median"],
					fold_summary["p25"],
		...
		if size_records:
			q_error_path, summary_path = _save_q_error_outputs(args, size, size_records, fold_summaries)
			if q_error_path is not None:
				print("Saved q_error details to {}".format(q_error_path))
			if summary_path is not None:
				print("Saved q_error summary to {}".format(summary_path))

		overall_q_summary = _summarize_q_errors(size_records)
		if overall_q_summary is not None:
			print("Summary for size {} - Test q_error mean: {:.4f}, median: {:.4f}, p25: {:.4f}, p75: {:.4f}".format(
				size,
	...
	for size in sorted(summary_by_size.keys()):
		...
		print("           Test q_error Mean {:.4f}, Median {:.4f}, P25 {:.4f}, P75 {:.4f}".format(
